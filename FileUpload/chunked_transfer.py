...
CLIENT_SECRETS_FILE = "D:\Projects\Project_TRO\FileUpload\client_secrets.json"
...
def upload(argv):
  ...
  while response is None:
    ...
    except getopt.HttpError as err:
      error = err
      if err.resp.status < 500:
        raise
    except getopt.RETRYABLE_ERRORS as err:
      error = err
...
def download(argv):
  ...
  print ("Downloading bucket: %s object: %s to file: %s" % (bucket_name, object_name, filename))
